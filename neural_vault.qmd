...
::: {.column width="50%"}
**Andrej Karpathy** — *The AI as Librarian*

Karpathy's core insight: have the LLM build and maintain a persistent wiki from raw sources.

*What he solved:*

- The `raw/` → `wiki/` ingestion pipeline
- LLM-written, human-curated notes
- Health checks to surface gaps
- Web Clipper ingestion for articles
:::

::: {.column width="50%"}
**What I Built — The Neural Vault** — *The AI as Collaborator*

I built this independently, solving a different problem: not how to store knowledge, but how to **load it into an active session**.

*What I built independently:*

- **Two-layer memory split**: Claude's ambient memory (who you are) + Obsidian wiki (what you know) — Karpathy doesn't address this
- **Context Injection Protocol**: a structured session-start handshake between your vault and the model
- **`COMMIT:` flag**: Claude marks insights worth saving mid-session
- **Auto-save daemon**: every save = a silent git commit. Zero manual steps.
:::
