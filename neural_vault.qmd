---
title: "The Neural Vault"
subtitle: "How to Build a Second Brain with Obsidian and AI"
author: "Abdullah"
date: "2026-04-10"
format:
  revealjs:
    theme: default
    transition: slide
    background-transition: fade
    slide-number: true
    embed-resources: true
    center: true
    css: style2.css
    incremental: true
---

## The Problem: AI Amnesia {.smaller}

**Every time you open a new chat, the AI starts from scratch.**

::: {.incremental}
- **Constant Re-explaining**: You spend the first ten minutes rebuilding context — your project, your conventions, your decisions, your background. Then the session ends. Next time, you do it again.
- **Nothing Accumulates**: The model is sharp but stateless. That statelessness is the real ceiling, not the model's intelligence.
- **Enterprise Solutions Are Overkill**: Companies use RAG and vector databases, but for personal use they are overbuilt, opaque, and fragile. You cannot debug a cosine similarity score by opening a markdown file.
:::

::: {.notes}
Start by asking the room: "Has anyone here ever had to re-explain yourself to an AI that should already know you?"

That's the problem. Every time you open a new Claude or ChatGPT session, the model has no memory of you. It doesn't know your name, your project, your preferences, or what you decided last week. You're starting from zero — every single time.

The frustrating part is that the model itself is incredibly capable. The bottleneck isn't intelligence. It's context. What you put in the window determines what you get out.

And before anyone says "just use RAG" — that's a valid enterprise solution, but it's massively overbuilt for one person. You don't need a vector database to remember your own notes. You need a system. That's what we're going to talk about today.
:::

---

## How This Started {.smaller}

**A conversation about markdown files sent me down a rabbit hole.**

::: {.incremental}
- A conversation with a professor about markdown note-taking workflows was the spark. I went home that night and started building.
- I was managing complex, multi-threaded technical work and kept hitting the same wall: re-explaining context every single session.
- The result is the **Neural Vault** — a structured Obsidian vault that works alongside Claude's native memory to give you persistent, portable, project-specific context across every AI session.
- No vector database. No infrastructure. Just markdown files, a disciplined workflow, and a few shell scripts.
:::

::: {.notes}
This system didn't come from a research paper. It came from a frustrating Tuesday night.

I was in a conversation about markdown files and note-taking — and something clicked. I went home and started building. Not a prototype. The actual thing. I had a specific problem: I was doing complex, multi-threaded work and every AI session felt like meeting a colleague who had amnesia.

What I built is called the Neural Vault. The name sounds technical, but the concept is simple: it's a notebook that the AI can read. A structured set of plain text files that you paste into Claude at the start of every session — so it already knows who you are, what you're working on, and what you've already decided.

No cloud. No API. No setup beyond installing a free app. Just files, a habit, and a few scripts if you want automation.
:::

---

## Inside the Obsidian Brain {.smaller}

**Your knowledge, connected and readable by both you and the AI.**

:::: {.columns}

::: {.column width="45%"}
<br>

- **Inbox**: Raw capture — quick, unorganized thoughts.
- **Wiki**: Structured knowledge the AI helped you write.
- **Meta**: `About_Me`, Index, Prompts, and Session Log.
- **Archive**: Deprecated notes, kept for history.
- **Agent Sandbox**: A messy workspace for AI agents — kept separate so your vault stays clean.
:::

::: {.column width="55%"}
![](obsidian_graph.png){width="100%" fig-align="center"}
:::

::::

::: {.notes}
So why Obsidian specifically? A few reasons — but the most important one is this: every note is a plain text file stored on your own computer. Not in a cloud. Not in a proprietary format. Just a `.md` file you can open in Notepad if you want.

That matters because LLMs read text. If your knowledge is in plain text files, you can hand it directly to an AI with no conversion, no API, no special integration. The format is already right.

The graph you're looking at is what Obsidian calls the "graph view" — it shows how your notes link to each other. Each dot is a note. Each line is a connection. As your vault grows, this becomes a visual map of your thinking.

The folder structure is simple: Inbox for raw capture, Wiki for structured knowledge, Meta for your About Me and session logs, Archive for old stuff, and Agent Sandbox for anything you let the AI write — kept separate so it doesn't pollute your curated notes.
:::

---

## Andrej Karpathy — *The AI as Librarian* {.smaller}

**His core insight: have the LLM build and maintain a persistent wiki from raw sources.**

:::: {.columns style="font-size: 0.8em;"}

::: {.column width="28%"}
![](karpathy.jpg){width="85%" style="border-radius: 8px; margin-top: 4px;"}
<div style="font-size: 0.75em; color: #64748b; text-align: center; margin-top: 4px;">Andrej Karpathy<br>Former Tesla AI Director<br>OpenAI co-founder</div>
:::

::: {.column width="72%"}
::: {.incremental}
- **The `raw/` → `wiki/` pipeline**: Raw notes go in; the LLM synthesizes structured wiki entries
- **LLM-written, human-curated**: AI does the writing, you do the editing
- **Health checks**: Periodic passes to find gaps and stale notes
- **Web Clipper ingestion**: Articles converted to markdown automatically
:::

. . .

> *Karpathy solved how to **build** a knowledge base. He didn't solve how to **use** it in an active session.*
:::

::::

::: {.notes}
Around the same time I was building this, Andrej Karpathy — if you don't know him, he's one of the most respected AI researchers in the world, co-founder of OpenAI, former head of AI at Tesla — published a very similar idea. The AI community went wild for it.

His insight was elegant: instead of manually writing your notes, have the LLM do it. You dump raw material — articles, highlights, rough thoughts — into a folder. The AI synthesizes them into clean, structured wiki entries. You review and approve. Over time, you build a knowledge base that the AI helped write.

It's a brilliant system for ingesting and organizing knowledge from what you read.

But here's the gap — and this is the key distinction I want you to hold onto — Karpathy's system tells you how to *build* the vault. It doesn't tell you how to *use* it in an active working session. How do you load it into Claude? How do you make sure the model has the right context before you start? That's what I built.
:::

---

## What I Built — The Neural Vault {.smaller}

**I started from a different problem: not how to store knowledge, but how to load it into an active session.**

:::: {.columns}

::: {.column width="45%"}
![](two_layer_diagram.png){width="100%" fig-align="center"}
:::

::: {.column width="55%" style="font-size: 0.82em;"}
::: {.incremental}
- **Two-layer memory split**: Claude holds *who you are*; Obsidian holds *what you know*. Karpathy doesn't address this.
- **Context Injection Protocol**: A structured prompt that loads your vault into the model at session start.
- **`COMMIT:` flag**: Claude marks insights worth saving mid-session — nothing gets lost.
- **Auto-save daemon**: Every file save = a silent git commit. Zero manual steps. Fully reversible.
:::
:::

::::

::: {.notes}
Here's what I built independently — and I want to be clear about what "independently" means here. I had the core of this system before Karpathy's post went up. When I read his work, I had to map carefully what we converged on, what he solved that I hadn't, and what I solved that he didn't.

The diagram on the left shows the key idea: a two-layer memory split. Layer one is Claude's native memory — the ambient stuff, who you are, your working style, your preferences. Layer two is your Obsidian wiki — the technical knowledge, project decisions, session history. These are different kinds of memory and they belong in different places.

The Context Injection Protocol is the bridge. It's a structured prompt template — a handshake — that you use at the start of every session to load the right parts of your vault into the model. Not everything. Just what's relevant.

The COMMIT flag is something I'm particularly proud of. Mid-session, when Claude surfaces a good insight, I've trained it to mark it with the word COMMIT. That's my signal to save it to the vault before the session ends. Nothing gets lost in the conversation.

And the auto-save daemon means every time I save a file in Obsidian, a shell script silently runs a git commit in the background. My entire vault is versioned. I can roll back to any point in time. Zero manual effort.
:::

---

## The Ultimate Setup: Both Together {.smaller}

**Use Karpathy's method to *build* the vault. Use Abdullah's method to *load* it.**

![](context_engineering.png){width="68%" fig-align="center"}

::: {.fragment}
<div style="font-size: 0.85em; color: #475569; text-align: center; margin-top: 10px;">
Karpathy fills the vault with knowledge from what you read. Abdullah's system loads that vault into every active work session — so the AI already knows your goals, your style, and what you decided yesterday.
</div>
:::

::: {.notes}
This is the synthesis slide — and this is the honest answer to "whose system is better?"

Neither. They solve different halves of the same problem.

Karpathy's method is the best way I've seen to build a knowledge base from raw material. His raw-to-wiki pipeline is genuinely elegant. If you read a lot and want the AI to help you synthesize it, use his approach.

My system is the best way I've found to load that knowledge into an active session and keep it updated as you work. The Context Injection Protocol, the two-layer split, the COMMIT flag — these are all about the live working session, not the archive.

The diagram here shows how context engineering works at a high level — how you take raw inputs and shape them into a final prompt that the model actually sees. The vault is your personal implementation of that pipeline.

Use both. Build with Karpathy. Load with the Neural Vault. That's the complete system.
:::

---

## The Result: Compounding Intelligence {.smaller}

**The system gets better every time you use it.**

::: {.incremental}
- **Stop Re-explaining**: The AI always knows your context because you hand it the ready-made summary.
- **Ideas Are Never Lost**: Every good session turns into a permanent note in your notebook.
- **You Own Everything**: Plain markdown files on your computer. No vendor lock-in. Switch models tomorrow — the vault comes with you.
- **An Investment in the Future**: The notebook you build today will make whatever AI you use tomorrow smarter and more aligned with you.
:::

. . .

> *"The vault you build today is the training set for the model you run tomorrow."*

::: {.notes}
Let me close with why this matters beyond productivity.

Most people think about AI tools as things you use. You open a chat, you ask a question, you get an answer, you close the tab. The interaction is disposable.

The Neural Vault flips that. Every session becomes an investment. Every good insight gets saved. Every decision gets recorded. Over weeks and months, your vault becomes a genuine external brain — and every AI session you have is better than the last because the context you bring to it is richer.

You also own everything. These are plain text files on your computer. If Claude shuts down tomorrow, your vault is intact. If a better model comes out next year, you bring your vault with you. No lock-in. No data held hostage.

The quote I want to leave you with is this: "The vault you build today is the training set for the model you run tomorrow." That's not a metaphor. That's literally what context engineering is. You are curating the information that shapes every future interaction. Start building now.
:::

---

## Access the Full System {.smaller}

**All scripts, templates, and documentation — open source on GitHub.**

:::: {.columns}

::: {.column width="55%"}
<br>

The complete Neural Vault system is available on GitHub, including:

- The full vault folder structure
- The Context Injection Protocol prompt template
- The `vault-autosave.sh` daemon script
- The `About_Me.md` starter template
- This presentation (`.qmd` source)

**github.com/rsm-aaljarallah**
:::

::: {.column width="45%"}
![](github_qr.png){width="65%" fig-align="center"}
:::

::::

::: {.notes}
Everything I've shown you today is on GitHub — free, open source, ready to use.

Scan the QR code or go to github.com/rsm-aaljarallah and find the neural-vault repo. You'll find the folder structure guide, the About Me template to fill in, the Context Injection Protocol prompt, and the auto-save script if you want to set up versioning.

The presentation itself is also in the repo as a Quarto `.qmd` file — so if you want to adapt it, fork it, or build on it, everything is there.

I'm happy to take questions. And if you want to go deeper on any part of this — the vault structure, the injection protocol, the auto-save setup — come find me after.
:::
